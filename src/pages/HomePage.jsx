  ...
  return (
    <>
      ...
      <section className="section-shell">
        <div className="grid gap-10 lg:grid-cols-2">
          <div>
            ...
            <h2 className="section-title">A permanent network of People of Action</h2>
            <p className="section-text">
              CHOPS International is an elite fellowship of Rotary leaders who continue the legacy of service through
              collaboration, leadership, and humanitarian work.
            </p>
            <p className="section-text">
              Even after leadership transitions, we remain committed to sustaining Create Hope outcomes globally.
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <h3 className="text-2xl font-semibold">Quick Membership Interest</h3>
            <p className="mt-2 text-slate-600">Join the fellowship and receive project and reunion updates.</p>
            <div className="mt-5 space-y-4">
              <input className="form-input" placeholder="Full name" />
              <input className="form-input" placeholder="Email address" type="email" />
              <textarea className="form-input min-h-24" placeholder="How would you like to contribute?" />
              <div className="flex flex-wrap gap-3">
                <button type="button" className="rounded-md bg-amber-400 px-5 py-3 font-semibold text-slate-900">
                  Submit Interest
                </button>
                <Link to="/membership" className="rounded-md border border-slate-400 px-5 py-3 font-semibold">
          ...
          {constructions.map((item) => (
            <article key={item.title} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="grid md:grid-cols-5">
                <img src={item.image} alt={item.title} className="h-56 w-full object-cover md:col-span-2 md:h-full" />
                <div className="p-6 md:col-span-3">
                  <h3 className="text-lg font-semibold text-amber-500">{item.title}</h3>
      ...
      <section className="section-shell">
        ...
        <h2 className="section-title">How CHOPS serves members and communities</h2>
        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {pillars.map((pillar) => (
            <article key={pillar.title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-amber-500">{pillar.title}</h3>
      ...
      <section className="section-shell bg-slate-100">
        <div className="grid items-center gap-10 lg:grid-cols-2">
          ...
          <div>
            ...
            <h2 className="section-title">Service momentum beyond one Rotary year</h2>
            <p className="section-text">
              CHOPS bridges fellowship and execution, helping chapters continue high-value service with strong peer
              support and shared frameworks.
            </p>
      ...
      <section className="section-shell bg-slate-100">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {gallery.map((item) => (
            <figure key={item.title} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <img
                src={item.image}
                alt={item.title}
                className="h-64 w-full object-cover transition duration-500 group-hover:scale-105"
              />
              <figcaption className="p-4 text-sm text-slate-700">{item.title}</figcaption>
            </figure>
          ))}
        </div>
      </section>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {blogs.map((post) => (
            <article key={post.title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-amber-500">{post.title}</h3>
              <p className="mt-3 text-slate-600">{post.snippet}</p>
              <Link to="/blog" className="mt-4 inline-block text-sm font-semibold text-amber-500">
                Read more
              </Link>
            </article>
          ))}
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {updates.map((item) => (
            <article key={item.title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-3 text-slate-600">{item.text}</p>
              <Link to="/news" className="mt-4 inline-block text-sm font-semibold text-amber-500">
                View details
              </Link>
            </article>
          ))}
